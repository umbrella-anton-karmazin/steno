...
- импортирует внешние аудио/видео файлы встречи;
...
- Sidebar:
  ...
  - список встреч;
  - индикатор занятого места библиотеки (`Занято: ...`);
  - кнопка Cleanup/Очистить;
  - кнопка Settings.
- Контент:
  - заголовок встречи;
...
- Загружаются:
  - основной медиа-файл встречи (для записи приложения это `.mp4`, для импортированных — один из поддерживаемых аудио/видео форматов);
  - `_mic.m4a` (если есть).
- При ошибке upload выполняется одноразовый fallback:
  - попытка normalize через `ffmpeg` во временный `.mp4`/`.m4a`;
  - повторная отправка в API;
...
- Источник: `save_dir`, по поддерживаемым первичным медиа-файлам (видео/аудио), сортировка `mtime desc`.
- Импортированные встречи копируются в библиотеку асинхронно с прогрессом.
- После импорта встреча фокусируется в UI и поднимается вверх списка (за счет обновления `mtime`).
...
- Cleanup библиотеки:
  - быстрые сценарии очистки файлов старше `7/30/90` дней (из sidebar и Settings);
  - перед удалением показывается подтверждение с оценкой количества записей и объема освобождаемого места;
  - при очистке удаляются связанные артефакты встречи (`<base>`, `_mic`, `_protocol`, `_protocol.meta`), активная запись/обработка не затрагивается.
...
## 8. Backlog (актуальные направления)
